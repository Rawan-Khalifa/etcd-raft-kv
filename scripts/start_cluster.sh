...
if [ ! -f "raft/proto/generated/raft_pb2.py" ]; then
...
# Start nodes
echo "Starting nodes..."
.venv/bin/python scripts/start_node1.py > node1.log 2>&1 &
NODE1_PID=$!
echo "  Started node1 (PID: $NODE1_PID, gRPC: localhost:9001)"

.venv/bin/python scripts/start_node2.py > node2.log 2>&1 &
NODE2_PID=$!
echo "  Started node2 (PID: $NODE2_PID, gRPC: localhost:9002)"

.venv/bin/python scripts/start_node3.py > node3.log 2>&1 &
NODE3_PID=$!
...
echo
echo "Commands:"
echo "  Monitor:  python scripts/demo_visualizer.py"
echo "  View logs: tail -f node1.log"
echo "  Stop:     ./scripts/stop_cluster.sh"
echo